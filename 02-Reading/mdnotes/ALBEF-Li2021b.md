...
- **图像编码器**：ViT-B/16，把图像切成 patch，得到 token 序列（含一个 [CLS]）；（补充：CLS 是一个 专门用来收集全局信息的“汇总槽位”：

在自注意力里，CLS 可以跟所有 patch 互相注意；

多层之后，CLS 就学会把“哪些 patch重要、如何组合”编码成一个向量。

这样，分类头只需要看 CLS，不用再对所有 patch 做平均 / 池化。

类比一下：

在 BERT 里有一个文本的 [CLS]，最后拿它来做句子分类；

在 ViT / CLIP-ViT 里就是“图像版的 [CLS]”。）
