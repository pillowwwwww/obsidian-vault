...
| 步骤  | 操作            | 示例                                        |
| --- | ------------- | ----------------------------------------- |
| 1   | 分词            | `"A dog jumps"` → `['a', 'dog', 'jumps']` |
| 2   | 添加 `<start>`  | `['<start>', 'a', 'dog', 'jumps']`        |
| 3   | 映射为索引         | `[1, 2, 3, 4]`                            |
| 4   | 替换部分为 `<unk>` | 如 `[1, 2, 1, 4]`                          |
| 5   | 删除若干 token    | 如 `[1, 2, 4]`                             |
| 6   | 添加 `<end>`    | `[1, 2, 4, 7]`                            |
| 7   | 转 Tensor      | `torch.Tensor([1, 2, 4, 7])`              |
|     |               |                                           |
|     |               |                                           |
              ...
              用户输入的文本 Caption（字符串）
                            ↓
                ┌─────────────────────────┐
                │    caption_transform()  │ ← 上层封装器
                │ 返回一个 Transform 对象 │
                └────────────┬────────────┘
                             ↓
       实际调用时会执行 → tokenize(sentence, vocab, caption_drop_prob)
                                ↑
         （由 partial() 注入 vocab 和 caption_drop_prob 参数）

## 二、噪音
caption_drop_prob 是一种数据增强参数，用于在训练过程中对文本 caption 进行扰动（噪声注入）。
它是一个 0~1 之间的浮点数，控制 丢弃或扰乱 token 的概率，以提升模型的泛化能力和鲁棒性。
