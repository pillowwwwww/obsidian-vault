...
---
修改后的两张图
Figure 1（原 Figure 2）—— Claim (1): FedAvg 产生显著稀释缺口

展示 δ_i 的分布，按 band 分层：

左面板：δ_i / σ_i（相对缺口）by band → 展示 tail 方向相对被稀释得更严重
右面板：δ_i（绝对缺口）by band → 展示各 band 的实际缺口大小
用 boxplot 或 violin，分 attention / FFN 两组。

Figure 2（原 Figure 3）—— Claim (2): 这些缺口有功能意义

因果干预：按 δ_i 从大到小排序，依次恢复 top-k 方向，测量性能回升。这里可以保留在 B 上操作，因为 A 近乎共享，修 B ≈ 修 ΔW。

叙事变为：
定义稀释缺口 δ_i（数学公式）
Figure 1：FedAvg 聚合后，ΔW 的 tail 方向产生大量 δ_i > 0（缺口存在且集中于 tail）
Figure 2：恢复 δ_i 最大的方向 → 性能立即提升（缺口有功能意义）
结论：值得在下一轮训练前修补 → 引出 FedDGC-B