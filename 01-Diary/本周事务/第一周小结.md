...
WHERE date(file.name)<date(2025-05-10) AND date(file.name) >= date(2025-05-02)
SORT file.name asc
```
...
```dataview
TABLE excerpt AS Summary, rating AS Rating
WHERE file.mday >= date(2025-05-02) AND status = "done"
WHERE file.mday < date(2025-05-10)
SORT rating desc
```
