...
- [ ] 短期目标：1.把研究计划的多模态联邦学习完善，不太确定选哪个方向，但是想先在多模态联邦学习这里进行研究
      2. 想把fl和大模型结合的框架复现一下
- [ ] 今天复现了FedAvg 但是还有一些细节需要看chatgpt的聊天记录，以及给chatgpt说，需要把自从上一个markdown之后的笔记整理一下
- [ ] 明天需要看一下这个软件怎么新建笔记，笔记放哪啊

## Tracking

- 22:47 python基础知识[[python基础知识]]
...
- 20:25 配置环境[[环境配置]]
	...
	##注意：可能无效，你需要临时开启权限： Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
...
- 请注意：总结下来就是一个项目是一个虚拟环境，使用venv或者是anaconda/miniconda。每个项目先配环境。
  切记不要使用pip install! 这会为整个环境安装包
  你需要使用! '''python -m'''
...
```
这是 PyTorch 的报错，意思是你当前的 PyTorch 是 **CPU-only 版本**，而你却调用了 `model.cuda()`。
如果使用 `venv`，你需要**手动指定带 CUDA 的 PyTorch 源地址**（就像你说的那条命令）。
